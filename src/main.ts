...
if (!["pipe", "geo", "rt", "code", "term", "chat"].includes(mode)) mode = "term";
...
function renderChatLog() {
  const box = chatLogEl();
  box.replaceChildren();
  if (!chatStore.log.length) {
    const empty = document.createElement("div");
    empty.className = "empty";
    empty.textContent = t("empty.chat");
...
async function sendChat() {
  ...
  try {
    ...
    for (const n of defaultSkills) {
      try {
        skillTexts.push(skillBody(await invoke<string>("read_skill", { name: n })));
      } catch {
        /* skip a skill that failed to read */
      }
    }
    const sys = [chatStore.system.trim(), ...skillTexts.map((s) => s.trim())].filter(Boolean).join("\n\n");
    await runWorker(
      chatStore.worker,
      sys,
      prompt,
...
function applyMode() {
  ...
  (document.querySelector(".results") as HTMLElement).classList.toggle("hidden", isTerm || isChat);
  $("#col-splitter").classList.toggle("hidden", isTerm || isChat);
