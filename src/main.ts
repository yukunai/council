...
const DEFAULT_DRAW: DrawState = { prompt: "", worker: "", ratio: "1:1", n: 1 };
...
function imageSourceOptions(): { value: string; label: string }[] {
  const opts: { value: string; label: string }[] = [];
  for (const c of clis) opts.push({ value: `cli::${c.name}`, label: `SVG · ${c.name}` });
  ...
  try {
    ...
    await Promise.all(
      cards.map(async (card, i) => {
        if (my !== genId) return;
        try {
          const res = await generateImage(draw.prompt.trim(), draw.worker, {
            size: ratio.size,
